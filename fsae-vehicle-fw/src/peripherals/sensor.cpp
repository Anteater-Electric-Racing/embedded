...
void threadSensor(void *pvParameters) {
    ...
    while(true) {
        ...
        shockTravel_mm = constrain((shockVoltage / 5.0f) * SHOCK_TRAVEL_MAX_MM, 0.0f, SHOCK_TRAVEL_MAX_MM);
        steeringAngle_deg = constrain((steerAngleVolt / 5.0f) * STEERING_ANGLE_MAX_DEG, 0.0f, STEERING_ANGLE_MAX_DEG); 
        steeringTorque_Nm = constrain((torqueVoltage / 5.0f) * TORQUE_SENSOR_MAX_NM, 0.0f, TORQUE_SENSOR_MAX_NM);
