...
void APPS_UpdateData(uint16_t rawReading1, uint16_t rawReading2) {  //changed uint16 from 32
    ...
    if (rawReading1 > APPS1_20PCT_ADC) rawReading1 = APPS1_20PCT_ADC;
    if (rawReading2 > APPS2_20PCT_ADC) rawReading2 = APPS2_20PCT_ADC;
    ...
    appsData.appsReading1_Percentage =
        LINEAR_MAP(appsData.apps1RawReading, (float)APPS1_REST_ADC, (float)APPS1_20PCT_ADC, 0.0F, 1.0F);

    appsData.appsReading2_Percentage =
        LINEAR_MAP(appsData.apps2RawReading, (float)APPS2_REST_ADC, (float)APPS2_20PCT_ADC, 0.0F, 1.0F);

    // clamp since LINEAR_MAP doesn't clamp
    appsData.appsReading1_Percentage = CLAMP01(appsData.appsReading1_Percentage);
    appsData.appsReading2_Percentage = CLAMP01(appsData.appsReading2_Percentage);
    ...
    // appsData.appsReading1_Percentage =
    //     LINEAR_MAP(appsData.apps1RawReading, 0.0F, (float)APPS1_20PCT_ADC, 0.0F, 1.0F);

    // appsData.appsReading2_Percentage =
    //     LINEAR_MAP(appsData.apps2RawReading, 0.0F, (float)APPS2_20PCT_ADC, 0.0F, 1.0F);
...
float APPS_GetAPPSReading() {
    return (appsData.appsReading1_Percentage + appsData.appsReading2_Percentage) / 2.0;
}
